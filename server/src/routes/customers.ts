...
router.get('/', async (req: Request, res: Response) => {
  try {
    const data = await splynx('get', '/customers/customer', undefined, {
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const data = await splynx('get', `/customers/customer/${req.params.id}`);
    res.json(data);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
router.get('/:id/services', async (req: Request, res: Response) => {
  try {
    const data = await splynx('get', `/customers/customer/${req.params.id}/internet-services`);
    res.json(data);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
router.get('/:id/invoices', async (req: Request, res: Response) => {
  try {
    const data = await splynx('get', `/billing/invoices`, undefined, {
